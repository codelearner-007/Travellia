...
const navLinks = [
  ...
  },
  {
    name: "Contact",
    path: "/contact",
    icon: "/navbar-icons/customer-service.png",
  },
];
      ...
      <ul className="hidden md:flex items-center gap-4 lg:gap-16">
        {navLinks.map((link) => (
          <li key={link.name} className="flex flex-col items-center gap-1">
            ...
            <Link
              href={link.path}
              className="text-sm lg:text-lg font-medium text-black dark:text-white hover:underline hover:text-primary"
            >
              {link.name}
            </Link>
          </li>
        ...
        <div className="absolute top-full left-0 w-full bg-gray-100 dark:bg-gray-800 z-50 flex flex-col items-center py-6 shadow-lg md:hidden border-t border-gray-200 dark:border-gray-700">
          <ul className="flex flex-col gap-6 w-full px-8">
            {navLinks.map((link) => (
              <li
                key={link.name}
                className="flex items-center gap-4 w-full border-b border-gray-200 dark:border-gray-700 pb-2 last:border-0"
              >
                <Image
                  src={link.icon}
                  alt={`${link.name} icon`}
